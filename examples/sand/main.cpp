...
int8_t gravity = 1;
int8_t percentInputFill = 10;
int8_t adjacentVelocityResetValue = 3;

int8_t inputWidth = 10;
int16_t inputX = 160; // COLS / 2;
int16_t inputY = 30;
// int16_t inputY = 85;  // ROWS / 2;

std::unordered_map<int32_t, int16_t> pixelColorNew;   // [X,Y]:[COLOR]
std::unordered_map<int32_t, int8_t> pixelStateNew;    // [X,Y]:[STATE]
std::unordered_map<int32_t, int8_t> pixelVelocityNew; // [X,Y]:[VELOCITY]

// static const int8_t GRID_STATE_NONE = 0;
static const int8_t GRID_STATE_NEW = 1;
static const int8_t GRID_STATE_FALLING = 2;
static const int8_t GRID_STATE_COMPLETE = 3;
...
void setup()
{
  ...
  pixelColorNew.reserve(COLS * ROWS);
...
void loop()
{
  ...
  fps = 1000 / std::max(currentMillis - lastMillis, (unsigned long)1);
    ...
    for (int16_t i = -halfInputWidth; i <= halfInputWidth; ++i)
    {
      for (int16_t j = -halfInputWidth; j <= halfInputWidth; ++j)
      {
        if (random(100) < percentInputFill)
        {
          ...
          int32_t xy = ((int32_t)col << 8) | row;

          if (withinCols(col) && withinRows(row) && pixelStateNew.find(xy) != pixelStateNew.end())
          {
            pixelColorNew.insert(xy, color);
            pixelStateNew.insert(xy, GRID_STATE_NEW);
            pixelVelocityNew.insert(xy, (int8_t)1);
          }
        }
      }
    }
